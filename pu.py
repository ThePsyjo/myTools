...
actionParser.add_argument('-g', '--gcloud-ssh', action='store', dest='dst_gcp', metavar='<target>', help='Connect via gcloud ssh. Use "list" to get a list of available targets.')
...
optionParser.add_argument('-H', '--show-host', action='count', dest='showHost', help='Show just the host if found')
...
optionParser.add_argument('-X', '--args', action='store', dest='args', default='', help='extra arguments')
...
try:
    config.read_file(open(parsed.configFile))
...
class Dataparser:
    ...
    def suggestTarget(self, keyword):
        ...
        if parsed.showHost:
            ...
            if parsed.showHost > 1:
                host = '%s@%s' % (self.getFromValue('user'), host)
            print (host)
            exit(0)
        #print self.value
    ...
    def getProject(self):
        return self.getFromValue('project')
    def getRegion(self):
        return self.getFromValue('region')

    def getFromConfig(self, key, default='', msg=True, exit_=True):
        try:
            return config.get('%s_Options' % self.section, key)
        except:
            if msg:
    ...
    def mkProjectArg(self, project=None):
        return self.fmt(self.getFromConfig('projectFmt'), project, self.getFromValue('project'), '')
    def mkRegionArg(self, region=None):
        return self.fmt(self.getFromConfig('regionFmt'), region, self.getFromValue('region'), '')
...
if parsed.dst_gcp:
    p.setSection('GCP')
    p.suggestTarget(parsed.dst_gcp)
    ...
    if parsed.pingHost:
        ...
        f(
            f'{p.getBin()} '
            f'{p.getFromConfig("ssh_command")} '
            f'{parsed.args} '
            f'{p.mkProjectArg()} '
            f'{p.mkRegionArg()} '
            f'{parsed.user or p.getUser() or ""}@{p.getHost()} '
            '-- '
            f'{p.getArgs()} '
            f'{p.mkPortArg(parsed.port)} '
            f'{parsed.ssh_remote_command if parsed.ssh_remote_command else ""}'
        )
...
if parsed.dst_ssh:
    ...
    if parsed.pingHost:
        ...
        f('%s %s %s %s %s %s %s' % (p.getBin(),
                p.getArgs(),
                p.mkPortArg(parsed.port),
                p.mkUserArg(parsed.user or p.getFromValue('user') or 'root'),
                parsed.args,
                p.getHost(),
